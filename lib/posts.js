...
// Merges Notion posts (primary) with markdown posts (fallback).
// Falls back to markdown readTime when Notion entry has none.
export function mergeAndSort(notionPosts, mdPosts) {
  const mdBySlug = Object.fromEntries(mdPosts.map((p) => [p.slug, p]));
  const notionSlugs = new Set(notionPosts.map((p) => p.slug));
  const enriched = notionPosts.map((p) => ({
    ...p,
    readTime: p.readTime || mdBySlug[p.slug]?.readTime || '',
  }));
  const mdOnly = mdPosts.filter((p) => !notionSlugs.has(p.slug));
  return [...enriched, ...mdOnly].sort((a, b) => parseDateTs(b.date) - parseDateTs(a.date));
}
